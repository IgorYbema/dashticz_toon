...
Item {
	...
	height: Math.round((64 + 54) ) * rowCount
	...
	width: parent !== null ? parent.width : (700 )
	...
	function addNewThumbnail(tileWidgetInfo) {
		...
		if (!thumbnailObj) {
			...
			DashticzJS.thumbnailObjsByWidget[tileWidgetInfo.uid] = thumbnailObj;
			thumbnailCount += 1;
			return true;
		}
	}
	...
	function removeThumbnail(uid) {
		var obj = DashticzJS.thumbnailObjsByWidget[uid];
		util.removeItem(thumbnailContainer, obj);
		DashticzJS.thumbnailObjsByWidget[uid] = undefined;
		thumbnailCount -= 1;
	}

	// Checks if the category contains a widget with the given uid
	function containsThumbnail(uid) {
		if (DashticzJS.thumbnailObjsByWidget[uid])
			return true;
		return false;
	}
	...
	function placeThumbnailRowAtBeginning(thumbnails) {
		for(var i = 0; i < thumbnails.length; i++) {
			util.placeComponentAt(thumbnails[i], thumbnailContainer, 0);
